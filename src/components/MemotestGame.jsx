...
export function MemotestGame ({ time, difficulty, setPlaying }) {
  ...
  return (
    <div className='w-full h-full relative grid grid-cols-[75%,25%] max-[750px]:grid-cols-none max-[750px]:grid-rows-[90%,10%]'>
      {
        lost && <LostMenu showingLost={showingLost} handlePlayAgain={handlePlayAgain} difficulty={difficulty} lost={true} />
      }
      {
        won && <LostMenu showingLost={showingLost} handlePlayAgain={handlePlayAgain} difficulty={difficulty} lost={false} timer={timer} time={time} />
      }
      ...
      <div className='bg-mainGray flex flex-col max-[750px]:flex-row max-[950px]:justify-center max-[950px]:justify-between max-[750px]:px-2'>
